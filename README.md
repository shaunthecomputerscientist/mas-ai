...
- **ToolRegistry**: Register hundreds of tools without bloating the LLM prompt
- **`discover_tools` Meta-Tool**: LLM discovers tools on-demand via keyword search
- **Lazy Execution**: Discovered tools are fetched from the registry and cached at runtime
- **Formatted Schemas**: Tool names, descriptions, and JSON input schemas returned in the exact `AnswerModel` format
...
Agent (Router-Evaluator-Reflector)
       ...
       ├─ Tool Executor (static tools + ToolRegistry fallback)
...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `tool_registry` | ToolRegistry | None | Registry for dynamic tool discovery. When set, a `discover_tools` meta-tool is auto-injected. |
...
agent = manager.create_agent(
    ...
    # tool_registry=my_registry  # See ToolRegistry section below
)
...
### Pattern 5: Agent with Dynamic Tool Discovery (ToolRegistry)

When you have many tools (tens or hundreds) and don't want to bloat the LLM prompt with all of them, use the `ToolRegistry`. The agent starts with only a `discover_tools` meta-tool. When the agent needs a tool, it searches the registry by keyword, discovers the tool's schema, and executes it dynamically.

```python
from masai.Tools.Tool import tool
from masai.Tools.ToolRegistry import ToolRegistry

# Define tools with the MASAI @tool decorator
@tool("get_weather")
...
def analyze_portfolio(holdings: dict, risk_level: str) -> str:
    """Analyzes a stock portfolio given holdings and risk tolerance."""
    return f"Portfolio analyzed with {risk_level} risk."

@tool("schedule_meeting")
def schedule_meeting(participants: list, duration_minutes: int) -> str:
    """Schedules a meeting for the given participants."""
    return f"Meeting scheduled for {len(participants)} people."

# Create registry and register all tools
registry = ToolRegistry(tools=[get_weather, analyze_portfolio, schedule_meeting])

# Create agent with NO static tools — only the registry
agent = manager.create_agent(
    agent_name="dynamic_agent",
    agent_details=AgentDetails(
        capabilities=["dynamic tool discovery"],
        description="Agent that discovers and uses tools on-demand"
    ),
    tools=[],              # Start with zero static tools
    tool_registry=registry # discover_tools meta-tool auto-injected
)

# The agent will:
# 1. Receive a query like "What's the weather in London?"
# 2. Call discover_tools(query="weather") to search the registry
# 3. Receive the schema: get_weather(location: str)
# 4. Call get_weather(location="London") dynamically
```

You can also **mix static and dynamic tools** — give the agent a few frequently-used tools directly, and put the rest in the registry:

```python
agent = manager.create_agent(
    agent_name="hybrid_agent",
    tools=[get_weather],         # Always available
    tool_registry=registry,      # analyze_portfolio, schedule_meeting discoverable
    agent_details=AgentDetails(
        capabilities=["tool execution"],
        description="Agent with both static and discoverable tools"
    )
)
```

### Pattern 6: Agent with Multiple Context Providers
...
### Pattern 7: Agent with Full Configuration
...
### Custom Tool Integration (MASAI Native)

MASAI provides its own `@tool` decorator — no LangChain dependency required:
...
> **Note**: LangChain tools (`from langchain.tools import tool`) are also fully supported. Both decorators produce compatible `Tool` objects.
...
The `ToolRegistry` class provides dynamic tool management:
...
registry = ToolRegistry(tools=[tool_a, tool_b, tool_c])

# Register additional tools later
registry.register_tool(tool_d)
registry.register_tools([tool_e, tool_f])

# Check tool existence
registry.has_tool("get_weather")  # True/False

# Fetch a specific tool by name
tool_obj = registry.get_tool("get_weather")  # Returns Tool or None

# Search tools by keyword (returns top_k matches)
results = registry.search_tools("weather forecast", top_k=5)

# Get the discover_tools meta-tool (auto-injected by create_agent)
meta_tool = registry.get_discovery_tool()  # Cached after first call
```

**How discovery works under the hood:**
1. `create_agent(tool_registry=registry)` auto-injects a `discover_tools` tool into the agent's static tools
2. The LLM calls `discover_tools(query="weather")` when it needs a tool
3. The registry searches by keyword and returns matching tool schemas
4. The LLM calls the discovered tool (e.g., `get_weather(location="London")`)
5. `base_agent.py` intercepts the call, fetches the tool from the registry, caches it, and executes it
...
- **ToolRegistry**: Dynamic tool registration, discovery, and lazy execution
- **Tool**: MASAI-native tool class with auto-generated Pydantic schemas
